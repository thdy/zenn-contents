...
:::message
2023.02.07 追記
本記事の内容はOkta Classic Engine環境を前提として書かれています。
Okta Identity Engine環境を対象としたAzure ADとのフェデレーションについてはこちらのクラウドネイティブさんの記事がおすすめです↓

> [Okta Identity EngineでのAzure ADとのフェデレーション設定はこれがおすすめだよ！ – CloudNative Inc. BLOGs](https://blog.cloudnative.co.jp/14430/)
:::
# はじめに
OktaをIdPとしてAzure AD(Microsoft 365)とフェデレーションを構成する際、[Okta公式のドキュメント](https://help.okta.com/en/prod/Content/Topics/Apps/Office365-Deployment/deploy-main.htm)通りだとうまくいかない部分があったり書いてないことがあったのでそのメモです。
...
# さいごに
記載内容に対する指摘や修正提案、不明点がある場合はコメント欄、GitHub、[Twitter](https://twitter.com/thdy_jp)等へお願いします。